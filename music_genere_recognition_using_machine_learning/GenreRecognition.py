...
def main():
    ...
    for p in path:
        print(p)
        x, sr = librosa.load(p, sr=sample_rate, duration=10.0)
        songs.append(x)
    ...
    data=[]
    for x in songs:
        y=extract_features(x, sample_rate, frame_size, hop_size)
        data.append(y)
    ...
    print("")
    print("----------------------------------------------------------------------------------------")
    createPlaylist.createFolders(preds)
    createPlaylist.moveSongs(preds)
